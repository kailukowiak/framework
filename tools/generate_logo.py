...
# Apple Icon Composer wants flat, opaque, transparent layers on one shared
# canvas: no tile, no rounded-rect mask, no baked shadows or gloss. The glass
# (specular, refraction, translucency, shadow) is added in Composer itself, so
# anything baked here would fight it. The canvas fill -- the sage tile in the
# shipped icon -- is set on the Composer canvas, not exported.
#
# The layers keep the shipped icon's seeded green wash per cell -- the same
# fills body_cells() produces for the raster/SVG masters -- but drop the
# emboss (no shaded copy) and every shadow/filter. Glass is added in Composer.
def _layer_svg(layer: str, desc: str, inner: str) -> str:
    return f'''<?xml version="1.0" encoding="UTF-8"?>
<!-- Generated by tools/generate_logo.py (icon-layers-dir); edit that, not this. -->
<svg xmlns="http://www.w3.org/2000/svg" width="{SIZE}" height="{SIZE}"
     viewBox="0 0 {SIZE} {SIZE}" role="img" aria-labelledby="title description">
  <title id="title">FrameWork app icon: {layer}</title>
  <desc id="description">{desc}</desc>
{inner}
</svg>
'''


def make_icon_layers(seed: int = CELL_SEED) -> dict[str, str]:
    """Transparent layers for Icon Composer, keyed by file stem.

    Numbered prefixes keep Z order on import: plate at the bottom, the FW
    letters on top. All three share the 512 canvas so they drop into place.
    Cell fills reuse body_cells(seed), so --seed varies these layers exactly
    like the shipped masters.
    """
    plate = (
        f'  <rect x="{TABLE_X:g}" y="{TABLE_Y:g}" '
        f'width="{TABLE_W:g}" height="{TABLE_H:g}" '
        f'rx="{TABLE_RADIUS}" fill="{PLATE}"/>\n'
        f'  <rect x="{INNER_X:g}" y="{TABLE_Y + OUTER_BORDER / 2:g}" '
        f'width="{INNER_W:g}" height="{HEADER_H - OUTER_BORDER:g}" '
        f'rx="{CELL_RADIUS}" fill="{HEADER}"/>'
    )
    data_cells = []
    letter_cells = []
    for x, y, fill, _, role in body_cells(seed):
        rect = (
            f'<rect x="{x:g}" y="{y:g}" '
            f'width="{CELL_W:g}" height="{BODY_CELL_H}" '
            f'rx="{CELL_RADIUS}" fill="{fill}"/>'
        )
        if role == "letter-cell":
            letter_cells.append(rect)
        else:
            data_cells.append(rect)
    return {
        "01-plate": _layer_svg(
            "frame plate",
            "The light grey frame plate and header band, no cells.",
            plate,
        ),
        "02-data-cells": _layer_svg(
            "data cells",
            "The non-letter grid cells with the seeded green wash.",
            "  " + "\n  ".join(data_cells),
        ),
        "03-letter-cells": _layer_svg(
            "FW letters",
            "The green grid cells spelling FW, with the seeded wash under the letters.",
            "  " + "\n  ".join(letter_cells),
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="export flat transparent SVGs for Apple Icon Composer into DIR",
    )
    ...
    if args.icon_layers_dir is not None:
        args.icon_layers_dir.mkdir(parents=True, exist_ok=True)
        for stem, text in make_icon_layers(args.seed).items():
            path = args.icon_layers_dir / f"{stem}.svg"
            path.write_text(text, encoding="utf-8")
            print(path)
