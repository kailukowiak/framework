...
#
# The layers are also flattened on purpose: the shipped icon varies every cell
# with a green wash and embosses each one, which turns to mud once refraction
# bends it. Here data cells are one solid CELL and letters one solid GREEN.
...
def make_icon_layers() -> dict[str, str]:
    """Flat transparent layers for Icon Composer, keyed by file stem.
    ...
    for row in range(BODY_ROWS):
        for col in range(COLS):
            ...
            rect = (
                ...
                f'rx="{CELL_RADIUS}" fill="'
            )
            if cell_is_letter(row, col):
                letter_cells.append(rect + f'{GREEN}"/>')
            else:
                data_cells.append(rect + f'{CELL}"/>')
    return {
        "01-plate": _layer_svg(
            "frame plate",
        ...
        "02-data-cells": _layer_svg(
            "data cells",
            "The non-letter grid cells in flat light grey.",
            "  " + "\n  ".join(data_cells),
        ),
        "03-letter-cells": _layer_svg(
            "FW letters",
            "The green grid cells spelling FW.",
            "  " + "\n  ".join(letter_cells),
        ),
    }
...
def main() -> None:
    ...
    if args.icon_layers_dir is not None:
        ...
        for stem, text in make_icon_layers().items():
