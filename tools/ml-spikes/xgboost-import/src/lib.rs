//! Restricted XGBoost 3.0 numerical gbtree JSON inference. No native runtime.
mod raw;
mod tree;
...
impl Model {
    pub fn from_json(bytes: &[u8]) -> Result<Self> {
        ...
        let ensemble: raw::Ensemble = serde_json::from_value(
            learner
                .gradient_booster
                .model
        ...
        Self::assemble(ensemble, features, groups, objective, base_margin)
    }
    ...
    /// The caller must provide the exported prediction policy. None explicitly
    /// means all rounds; best_iteration attributes never silently change it.
    /// Columns are in trained positional order. Null/NaN means missing.
    pub fn predict(&self, row: &[Option<f64>], range: Option<IterationRange>) -> Result<Vec<f32>> {
        require(
        ...
        let range = range.unwrap_or(IterationRange {
            start: 0,
            end: self.rounds(),
        });
