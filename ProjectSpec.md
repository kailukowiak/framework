...
2. **Repeatable data access.** Move beyond linked CSV/TSV/Parquet files to
   ...
   Schema change landed 2026-09-02: a refresh reconciles fields by source
   name, a vanished field that something still reads empties only its own
   column (a typed null column carrying a per-column error) rather than
   failing the frame, and the refresh returns a `SchemaDiff` (added, removed,
   kept without data and why, type changed) that the Source panel reports.
...
- **Read is the start of a frame (2026-09-09).** Wrangle exposes the input before transformations, including entered data. Paste a complete set of column names into one editing surface; a batch rename is one undoable operation. File-backed working copies and artifact frames can replace their input with a file or database query while retaining matching column identities and transformations. Writing out leaves that source and transformation chain intact and brings the new values-only file back as a separate clean table. Saving the notebook is what makes the original recipe reusable; discarding it leaves only the output file. CSV inference is conservative on the paged path too: ordinary numbers remain numeric, but leading-zero identifiers, long digit strings and ambiguous dates stay text. Type overrides are editable in Read through one shared column/type picker (Text, Integer, Number, Boolean, Date). They are ordinary same-column Polars casts before authored transformations, retained in the visible chain; “From source” removes the override. The inspector starts collapsed and grid formula edits keep that state, with their shared editor in the one-row top bar. Multiline expansion is explicit so selecting a formula does not move the grid beneath the next pointing gesture.
- **Editable CSV/TSV and explicit output (2026-09-07, storage revised 2026-09-09).** A stored CSV/TSV is staged into a parquet the workbook reads and is typed into by recording corrections against that read — a cell, a struck-out row, a row added past the end — so an edit costs the edit and the file is left exactly as written. There is no size ceiling and no separate read-only fallback: the 20,000-row cap existed only because an opened file used to be copied into the document as literal cells. The frame's *Write out CSV…* action chooses a new file, writes the full data result—including renamed headers, manual edits and Wrangle steps—and adds that file to the canvas as an imported table with no transformations. The working table is unchanged, so its recipe remains reusable if the notebook is saved. *Update original CSV…* is the confirmed in-place alternative: it atomically replaces the source without keeping a recovery copy, adds the replacement as a new direct-read frame with no transformations, and leaves the original working table and its active transformation chain intact. Saving the workbook keeps that recipe for reuse; closing without saving discards the workbook-side split but does not reverse the external overwrite. Both are deliberately separate from ⌘S and workbook autosave. Header filters and sorts are pipeline operations; only formatting and legacy display steps are excluded. Undo changes workbook state but never silently rewrites an external file. The exact fidelity contract and current import limits are in [CSV editing](docs/CSV_EDITING.md). This supersedes the static-import paragraph below for stored CSV/TSV only.
...
shown in the ordinary dense editable grid. The UI calls it a mapping frame,
not a separate object type. It stores no second copy of its
entries and needs no new document object kind. Add dictionary on the canvas
creates Key and Value columns; Use for mapping marks the selected column of
...
Map values also offers Create mapping frame directly, named for the selected
column. Fill its Key/Value grid, then choose it from the original column's
Map values menu; creation alone does not apply an empty mapping. One hint
explains that blank keys match missing values and blank replacements produce
missing values. Otherwise offers keep original, leave blank, or custom text.
Rename columns sits beside Map values and resolves a keyed mapping frame's
complete data layer into one atomic, undoable rename. Unmatched headers remain;
blank or duplicate resulting names fail without a partial edit. Header renaming
is a one-time edit, whereas mapped cell values remain live formulas.
