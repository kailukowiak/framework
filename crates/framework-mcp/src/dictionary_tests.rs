...
fn matrix_input_bindings_use_the_canonical_operation_surface() {
    ...
    let (input, matrix) = {
        ...
        (
            objects
                .iter()
                .find(|o| o.name() == "growth")
                .unwrap()
                .id()
                .to_string(),
            objects
                .iter()
                .find(|o| o.name() == "Sensitivity")
                .unwrap()
                .id()
                .to_string(),
        )
    ...
    assert_eq!(
        view.computed_calculation_matrices[&matrix].cells[1][0].value,
        Some(30.0)
    );
...
fn parameter_values_use_the_canonical_typed_operation() {
    ...
    let object_id = server.lock().unwrap().store.view().parameter_inputs[0]
        .id
        .clone();
    server.apply_operation(Parameters(ApplyOperationArgs {
    ...
    assert_eq!(
        server.lock().unwrap().store.view().computed_results[&object_id]
            .cell
            .value,
        Some(0.25)
    );
...
fn dictionaries_are_available_through_the_canonical_operation_catalog() {
    ...
    let frame = session
        ...
        .find_map(|object| match object {
            DataObject::Frame(frame) if frame.name == "Corrections" => Some(frame),
            _ => None,
        })
        .unwrap();
    assert_eq!(
        frame.unique_keys[0].column_ids,
...
fn mapping_rename_is_discoverable_and_uses_the_canonical_operation() {
    ...
    assert!(
        server
            ...
            .0
            .type_script
            .contains(r#""type": "renameColumnsUsingMapping""#)
    );
    for operation in [
...
fn period_declaration_is_discoverable_and_uses_the_canonical_operation() {
    ...
    assert!(
        server
            ...
            .0
            .type_script
            .contains(r#""type": "setFramePeriod""#)
    );
    server.apply_operation(Parameters(ApplyOperationArgs {
    ...
    assert_eq!(
        declared.period.as_ref().unwrap().column_id,
        frame.columns[0].id
    );
    drop(session);
    if path.exists() {
...
#[test]
fn calendars_are_discoverable_and_use_the_canonical_operations() {
    let (server, path) = test_server();
    assert!(
        server
            .describe_operations()
            ...
            .0
            .type_script
            .contains(r#""type": "addCalendar""#)
    );
    server.apply_operation(Parameters(ApplyOperationArgs {
            operation: serde_json::json!({"type":"addCalendar","name":"NRF","fyStart":2,"pattern":"fourFiveFour","yearEnd":{"nearestWeekday":{"weekday":6,"month":1,"day":31}},"yearLabel":"start","weekend":[6,7],"holidays":[]}), expected_revision: None,
        })).unwrap();
    let id = {
        let session = server.lock().unwrap();
        assert_eq!(session.store.document().calendars.len(), 1);
        session.store.document().calendars[0].id.clone()
    };
    server
        .apply_operation(Parameters(ApplyOperationArgs {
            operation: serde_json::json!({"type":"setDefaultCalendar","calendarId":id}),
            expected_revision: None,
        }))
        .unwrap();
    let session = server.lock().unwrap();
    assert_eq!(
        session.store.document().default_calendar_id,
        Some(id.clone())
    );
    drop(session);
    if path.exists() {
