use super::{Prediction, import, proto, wire};
use crate::pipeline_types::{
    ClassLabel, MissingValue, NumericPrecision, Scalar, ScaleOperation, Step,
};
use prost::Message;
use serde_json::Value;

const LINEAR: &[u8] = include_bytes!("../../tests/fixtures/onnx/branched-linear.onnx");
const LOGISTIC: &[u8] = include_bytes!("../../tests/fixtures/onnx/branched-logistic.onnx");

fn fixture(classification: bool) -> Value {
    serde_json::from_str(if classification {
        include_str!("../../tests/fixtures/onnx/branched-logistic.json")
    } else {
        include_str!("../../tests/fixtures/onnx/branched-linear.json")
    })
    .unwrap()
}
...
#[test]
fn saved_pipeline_roundtrip_preserves_predictions_and_rejects_corruption() {
    for bytes in [LINEAR, LOGISTIC] {
        let model = import(bytes).unwrap();
        let serialized = serde_json::to_value(&model).unwrap();
        let restored: super::ImportedLinear = serde_json::from_value(serialized.clone()).unwrap();
        assert_eq!(restored, model);
        let input = rows(&fixture(bytes == LOGISTIC));
        assert_eq!(
            model.predict(&input).unwrap(),
            restored.predict(&input).unwrap()
        );
        for field in ["coefficients", "intercepts", "inputs"] {
            let mut corrupted = serialized.clone();
            corrupted[field] = serde_json::json!([]);
            let bad: super::ImportedLinear = serde_json::from_value(corrupted).unwrap();
            assert!(bad.predict(&input).is_err(), "{field}");
        }
        let mut corrupted = serialized;
        corrupted["recipe"]["features"] = serde_json::json!([]);
        let bad: super::ImportedLinear = serde_json::from_value(corrupted).unwrap();
        assert!(bad.predict(&input).is_err());
    }
}
