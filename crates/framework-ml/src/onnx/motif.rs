...
fn tensor(value: &Value, dtype: i32, width: Option<i64>) -> Result<()> {
    let tensor = value
        ...
        .ok_or_else(|| super::Error("missing tensor type".into()))?;
    ensure(tensor.elem_type == Some(dtype), "unsupported tensor dtype")?;
    let dims = &tensor
        .shape
        .as_ref()
        .ok_or_else(|| super::Error("missing shape".into()))?
        .dims;
...
fn names(graph: &Graph) -> Result<()> {
    ...
    {
        let name = name.ok_or_else(|| super::Error("missing input/initializer name".into()))?;
        ensure(
...
pub fn check(model: &Model) -> Result<(&Graph, bool)> {
    ...
    let g = model
        .graph
        .as_ref()
        .ok_or_else(|| super::Error("missing graph".into()))?;
    ensure(
