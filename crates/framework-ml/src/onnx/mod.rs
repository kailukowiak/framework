//! Bounded ONNX pipeline translator. Only the validated sklearn branched linear and
...
use crate::pipeline_types::{ClassLabel, Input, Recipe};
use prost::Message;
...
pub fn import(bytes: &[u8]) -> Result<ImportedLinear> {
    ...
    let fitted = lower::lower(graph, classification)?;
    fitted.validate()?;
    Ok(fitted)
}
