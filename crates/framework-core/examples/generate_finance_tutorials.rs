...
/// The seed is written `400000.0` on purpose: a recurrence keeps the type of
/// its first value, and an integer seed would truncate every balance to
/// whole dollars. The README says so beside the line.
const LOAN_TERMS: &str = "principal = 400000.0\n# 400000.0 rather than 400000: a recurrence keeps the type of its first value, and the schedule needs cents\nrate = 0.06\nterm = 60";

fn generate_price_a_deal(output: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    store.apply(Operation::SetFramePipeline {
        ...
        steps: vec![
            ...
            with_columns(&[("Interest", "`Opening` * `Loan terms`.monthly")]),
            with_columns(&[("Principal paid", "`Loan terms`.payment - `Interest`")]),
    ...
    // Section 3: XNPV from parts.
    let deal_y = loan_y + view_height(&store, &object_id_named(&store, "Loan terms")) + 40.0;
    add_block(&mut store, "Deal", 720.0, deal_y, "rate = 0.10")?;
    let flows = frame(&store, "Cash flows");
    store.apply(Operation::SetFramePipeline {
        frame_id: flows.id.clone(),
        steps: vec![
            with_columns(&[(
                "Years",
                "(`Date` - `Date`.min()).dt.total_days(False) / 365",
            )]),
            with_columns(&[("Discount factor", "1 / (1 + `Deal`.rate) ** `Years`")]),
            with_columns(&[("Present value", "`Amount` * `Discount factor`")]),
        ],
    })?;
    format_columns(&mut store, "Cash flows", &["Present value"], money_format())?;
    set_block(
        &mut store,
        "Deal",
    ...
    // Section 4: the rate scan that stands in for a solver. A block rather
    // than a Calculation Matrix: a matrix body cannot read a table that has
    // calculated columns unless that table is materialised, and a block can.
    let scan_y = deal_y + view_height(&store, &object_id_named(&store, "Deal")) + 40.0;
    let scan = [5, 10, 15, 20, 25, 30]
        .iter()
        .map(|rate| {
            format!(
                "`NPV at {rate}%` = (`Cash flows`.`Amount` / (1 + 0.{rate:02}) ** `Cash flows`.`Years`).sum()"
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    ...
    let flows = page(&reloaded, "Cash flows");
    for (row, years) in flows
        .rows
        .iter()
        .zip([0.0, 0.454795, 0.958904, 1.454795, 1.958904, 2.457534])
    {
        assert_close(&row[2], years, &format!("{} Years", row[0]));
    }
