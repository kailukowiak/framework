...
- Arithmetic between an accounting amount and another number stays exact end to end: a whole number joins as-is, a number written into the formula keeps its own decimal places, a plain number column is given nine places to work with, and the result lands at whichever side's scale is wider.
