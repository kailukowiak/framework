...
- Ask what a result would be under a scenario without switching into it: `under(`Upside`, ebitda)` reads the scenario's assumptions on a private copy of the document and answers, while the canvas stays on Base. It works on a Scratchwork line, in a calculated column, in a result and in a Calculation Matrix body, so a comparison table is a few lines rather than numbers copied off the screen. A scenario named this way follows a rename and cannot be removed while a formula reads it, the same rule a value already has; a name no scenario answers to lists the scenarios the document does have.
- Goal seek: `solve(`Model`.ebitda == 300000, by=`Price`, within=[100, 200])` finds the price that hits the target by bisection and answers `138.75`. On its own Scratchwork line the gutter shows the steps taken and the residual, with an Apply action that sets the value as an ordinary, undoable edit. A bracket with no crossing is refused with both end values named rather than answered with the nearest miss, and a whole-number value can be varied continuously. Inside arithmetic, `solve(...)` is just its number.
- A sensitivity grid (a Calculation Matrix axis bound to a value or named control) now says how many times it evaluated the model and how long that took, so a grid over a large model shows its cost instead of hiding it.
- Right-click a formula block in a document that has scenarios and choose **Compare scenarios**: FrameWork builds a table beside it with one row per named line and one column per scenario, Base first. Each column reads the same block through that scenario's assumptions, so Upside and Downside sit side by side without switching the document into either one. The result is an ordinary table — sort it, format it, plot it, or add columns of your own. It is two edits today, so undo takes two steps.
- The *Scenarios, sensitivity and goal seek* tutorial's final section now runs for real: the two `under()` comparison lines, a sensitivity grid bound to `Price` and `Annual units` reading `` `Model`.ebitda `` directly, and a `solve()` goal seek for the price that hits a target EBITDA, with the answer key rebuilt to match.
- Agents can create a scenario in one call: the MCP `add_scenario` tool takes a name and the values it disagrees with the base about, without switching the document onto it.
...
- Arithmetic between an accounting amount and another number stays exact end to end: a whole number joins as-is, a number written into the formula keeps its own decimal places, a plain number column is given nine places to work with, and the result lands at whichever side's scale is wider. Wherever an amount is rounded to fewer places it rounds half to even, in a typed cell and a computed one alike.
- Fixed: undoing a column's type change used to skip past it when the column had no categories, landing on the edit before it instead. Undo now always reaches the retype itself.

- Accounting is offered wherever a column type is chosen, including Wrangle's read types, with the column's decimal places set beside it in the inspector. An accounting column counts as a number for totals, charts, joins, drag-fill and conditional formatting, and a cell accepts `(12.50)` or `$1,234.56` as typed.
- Amounts survive the trip out: an accounting column exports as an exact decimal to CSV and Parquet and as a real number to Excel, writing back to a CSV keeps the cents it held rather than the nearest float, and a Parquet file's own decimal places become the column's declared places on the way in. `.cast` offers `"accounting"` in completion and the catalog, and the MCP tools describe the type, its places and a column's scale, so an agent building a ledger can make it foot exactly. A `ledger` smoke scenario checks that it does.
- A paired list can now repeat to fill the frame: pair five scenario values down a hundred-row frame and each row gets the next value, round and round, with the frame's row count required to be a whole multiple of the list's length. *Exact*, one value per row, stays the default, and each refusal names both counts and the other choice.
- A join can match on several key columns together — account, department and period as one composite key — where it took one column a side before. The key pairs travel side by side, and a lopsided list is refused by count.
- Agents get the same vocabulary a person has: a `combine_frames` tool that says how two tables' rows relate (match on keys, same rows in order with an exact or repeating list, rows underneath, or every row with every row) and a `sort_frame` tool that declares the row order a running balance needs, which the ledger scenario had to reach for through a raw pipeline edit.
- Join is now Combine. One dialog asks how two tables' rows relate and takes four answers: match on a key, same rows in order, rows under rows, and every row with every row. The first makes a joined frame as before; the other three append a pair, stack or expand step to the table's own chain, with the row counts, the name-matched columns and any type disagreement shown before the step is saved. The header-onto-header drag still opens the compact lookup prompt first, and its *More options* lands in Combine.

- A prediction frame is now a scored table: the scoring frame's columns with the model's outputs added at the end, instead of the outputs on their own. That gives the predictions a key, so they can be joined, filtered and built on like any other frame; delete the carried columns in Wrangle if only the outputs are wanted. The card opens wide enough for its columns, and the frame follows its scoring frame from then on — a column added, renamed, retyped or removed there is added, renamed, retyped or removed in the prediction frame too, undo included, while a name or format chosen on the prediction frame itself is kept.
- Wrangle's *Pair vector as column* and *Apply vector across columns* can name a column of another frame that has no snapshot — `` `Predictions`.`Prediction` `` — and read it live. The completion menu offers every frame there, clicking a column on another card inserts its qualified name, and dragging a column's header onto another table's **+** edge pairs it there in one gesture. Naming a frame built from the one being edited is refused as a loop, with the fix named; a loop that arrives some other way is reported instead of hanging.
- A paired list is checked against the rows the frame has *now*, not the count it had when the step was written, so predictions paired beside a scoring frame keep holding as that frame grows. A list that no longer has one value per row is refused with both counts. A column formula that names a frame without a snapshot is now told about the pair step as well as about materializing.
- Deleting a column that a derived frame selects, sorts by, pivots on, melts or spreads a list across is refused by name, the way a column a formula reads already was.
- `.head(n)` and `.tail(n)` are in the completion menu and the catalog.
- The prediction dialog for an imported model lists the columns it was mapped to by their own names, rather than the file's `feature_1`, `feature_2` names that no frame answers to; choosing a different source frame lists the fitted names as the order to match.
- Renaming a fiscal calendar now just works: formulas that name it follow the rename and keep reading the same calendar, instead of the rename being refused. Removing a calendar a formula reads is refused the way deleting a value someone reads is, naming the frame or line that reads it, and a calendar name no calendar answers to is reported as you write the formula, listing the calendars the document has.
- Year-to-date and trailing-twelve-month columns no longer slow down as a frame grows. A window used to pair every row with every other row in its partition before narrowing to its twelve periods; it now asks for exactly the periods it covers, so a five-thousand-row daily frame with one `ttm` column went from seconds to a fraction of one.
- Period-relative functions now count in a calendar. `period_index`, `prior`, `ytd`, `ttm` and `same_period_last_year` take an optional `calendar=`, and with none written they read the document's default calendar — the same one `fiscal_period` reads — so a bare `period_index` or `prior` no longer counts from January beside a `fiscal_period` counting from the default calendar's year start. Under a retail week calendar the windows count retail blocks: `ytd` resets on the retail year start, and `ttm`, `prior` and `same_period_last_year` step whole retail periods. Writing `fy_start` together with a retail calendar is refused rather than silently dropped.
- Fiscal calendars are harder to break: `fiscal_week` no longer crashes under a calendar-months calendar, a 4-4-5, 4-5-4 or 5-4-4 calendar is refused if its year end would leave a part week, February 29 is refused as a year-end anchor, a calendar must keep at least one working day, and `workday` reads blank rather than hanging on an impossible shift. A calendar's `weekend` lists the days *not* worked — the documentation said the opposite.
- Renaming or deleting a fiscal calendar is refused while a formula still names it, and the refusal says which frame, result or Scratchwork block reads it. A calendar that cannot be read is reported wherever it is used, including as the document default, instead of quietly falling back to January months. Undoing the removal of the default calendar restores it as the default, and calendar names given to the MCP tools match without regard to case, the way formulas do.
- Name fiscal calendars on the document — year start, 4-4-5/4-5-4/5-4-4 or calendar-month patterns, the year-end rule, year labelling, weekends and holidays — with add, update, remove and set-default operations. Fiscal functions take an optional calendar per call and bare calls read the default; `fiscal_week` numbers retail weeks matching the published NRF calendar, and `workday`/`networkdays` count business days around weekends and holidays. The frame menu declares a date column as the period in one choice.
